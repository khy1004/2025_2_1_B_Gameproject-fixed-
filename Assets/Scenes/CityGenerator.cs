...
public class CityGenerator : EditorWindow
{
    ...
    public void OnGUI()
    {
        ...
        gridSizeX = EditorGUILayout.IntField("gridSizeX", gridSizeX);
        gridSizeZ = EditorGUILayout.IntField("gridSizeZ", gridSizeZ);
    ...
    private void CreateBuiling(Vector3 position , Transform parent)
    {
        ...
        renderer.material.color = new Color(Random.Range(0.5f, 0.8f), Random.Range(.5f, 0.8f), Random.Range(.5f, 0.8f));
    ...
    private void GenerateCity()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            ...
            {
                Vector3 pdsition = new Vector3(x * buildingSpacing, 0, z * buildingSpacing);

                if (x% 2 == 0 || z % 2 == 0)
                {
                    CreatRoad(pdsition, roadsParent.transform);
                }
                else
                {
                    CreateBuiling(pdsition, buildingsParent.transform);
                }
            }
        }
